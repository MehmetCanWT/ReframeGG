  ...
  return (
    <div className="min-h-screen bg-[#070809] text-zinc-100 overflow-x-hidden selection:bg-pink-500 selection:text-white">
      ...
      <header className="border-b border-white/5 bg-[#0b0c0e]/60 backdrop-blur-md sticky top-0 z-50 transition duration-300">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8 text-xs font-black tracking-widest uppercase text-zinc-400">
            ...
            <a href="#screenshot" className="hover:text-pink-500 transition">Interface</a>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 pt-20 pb-16 text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-xs font-semibold text-zinc-300 mb-6 backdrop-blur-sm animate-pulse">
      ...
      <section id="screenshot" className="relative z-10 max-w-6xl mx-auto px-6 py-12">
        <div className="bg-[#0b0c0e]/80 border border-white/10 rounded-2xl p-3 shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden">
          ...
          <div className="flex items-center justify-between border-b border-white/5 pb-3 px-3 mb-3 select-none">
            ...
            <div className="text-[10px] font-black text-zinc-500 uppercase tracking-widest flex items-center gap-1">
              ...
              ReframeGG - Compositor Workspace
            </div>
            <div className="w-[40px]" />
          </div>

          {/* App Screenshot Placeholder */}
          <div className="relative aspect-video w-full bg-zinc-950/60 rounded-xl overflow-hidden flex items-center justify-center border border-white/5">
            {/* Fallback mockup UI if image is not placed yet */}
            <div className="absolute inset-0 z-0 flex flex-col items-center justify-center p-8 text-center bg-[radial-gradient(ellipse_at_center,rgba(236,72,153,0.06),transparent_80%)]">
              <Image 
                src="app_screenshot.png"
                alt="ReframeGG Desktop Interface"
                fill
                priority
                className="object-cover opacity-90 transition hover:opacity-100 duration-500"
                onError={(e) => {
                  // Hide image on error so standard fallback mock displays cleanly
                  (e.target as HTMLElement).style.display = 'none';
                }}
              />
              
              <div className="relative z-10 flex flex-col items-center max-w-md pointer-events-none">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-pink-600 to-fuchsia-500 flex items-center justify-center shadow-lg shadow-pink-600/30 mb-5">
                  <Sparkles size={28} className="text-white animate-pulse" />
                </div>
                <h3 className="text-lg font-black uppercase text-white mb-2 tracking-wide">Workspace Viewport</h3>
                <p className="text-zinc-500 text-xs font-semibold leading-relaxed mb-4">
                  Place your own desktop application screenshot named exactly <code className="text-pink-500 font-mono text-[10px] px-1 py-0.5 bg-white/5 rounded">app_screenshot.png</code> inside the <code className="text-pink-500 font-mono text-[10px]">github-pages/public/</code> directory.
                </p>
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded bg-pink-500/10 border border-pink-500/20 text-[9px] font-black text-pink-500 uppercase tracking-widest">
                  Ready to Load App View
                </div>
              </div>
            </div>
          </div>
