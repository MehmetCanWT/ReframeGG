...
export function FlowEditor({
  ...
  const getMaskBase64 = (layer: ReframeLayer): string | undefined => {
    ...
    layer.masks.forEach(m => {
      ...
      if (m.type === "circle") ctx.ellipse(lx + m.w/2, ly + m.h/2, m.w/2, m.h/2, 0, 0, Math.PI * 2);
      else if (m.type === "freeform" && m.points) m.points.forEach((p, i) => i === 0 ? ctx.moveTo(p.x - layer.cropArea.x, p.y - layer.cropArea.y) : ctx.lineTo(p.x - layer.cropArea.x, p.y - layer.cropArea.y));
      else ctx.rect(lx, ly, m.w, m.h);
      ctx.fill();
    });
  ...
  const handleSavePreset = async () => {
    ...
    const newPreset: Preset = {
      game: "Custom", presetName: name, sourceResolution: { w: 1920, h: 1080 },
      layers: layers.map(l => ({
        ...
        locked: false, visible: l.visible, blur: l.blur, brightness: l.brightness, contrast: l.contrast
      }))
    };
    // V2 Preset stores the full layer object as extra metadata if needed, but for now we follow the schema
  ...
  const applyPreset = (preset: Preset) => {
    const mapped: ReframeLayer[] = preset.layers.map((l, idx) => {
      ...
      const type = (l.maskShape === "circle" || l.maskShape === "freeform") ? "mask" : "crop";
      
      const masks: MaskShape[] = type === "mask" ? (l.masks && l.masks.length > 0 ? l.masks : [
        {
          id: `mask_preset_${idx}`,
          name: l.label,
          type: l.maskShape === "circle" ? "circle" : "freeform",
          x: l.cropArea.x,
          y: l.cropArea.y,
          w: l.cropArea.w,
          h: l.cropArea.h,
  ...
  useEffect(() => {
    const handleGlobalMove = (e: MouseEvent) => {
      ...
      const nextX = snapValue(Math.round(silDragRef.current.startLayerX + dx), [0, 540 - lw/2, 1080 - lw], 50);
      const nextY = snapValue(Math.round(silDragRef.current.startLayerY + dy), [0, 960 - lh/2, 1920 - lh], 50);
  ...
  }, [selectedLayerId, selectedLayer, layers, handleUpdateLayer, snapValue, pushHistory, setEditorTool]);
    ...
    <div className="w-screen h-screen flex flex-col bg-[#0b0c0e] text-[#e4e4e7] overflow-hidden select-none font-sans">
      ...
      {showPresetLibrary && (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/90 backdrop-blur-xl">
          <div className="w-[800px] max-h-[80vh] bg-[#111215] border border-white/10 rounded-3xl p-8 shadow-2xl flex flex-col">
            <div className="flex justify-between items-center mb-8"><h2 className="text-xl font-black text-white uppercase tracking-tighter">Preset Library</h2><button onClick={() => setShowPresetLibrary(false)} className="text-zinc-500 hover:text-white transition cursor-pointer font-black text-[10px] tracking-widest">CLOSE</button></div>
            <div className="flex-1 overflow-y-auto grid grid-cols-2 gap-4 custom-scrollbar">
              {[...defaultPresets, ...customPresets].map((p, i) => (
                <div key={i} onClick={() => applyPreset(p)} className="p-5 rounded-2xl bg-[#16181d] border border-white/5 hover:border-orange-500 transition cursor-pointer group flex flex-col gap-2">
                  <div className="text-[13px] font-black text-white group-hover:text-orange-500 uppercase tracking-tight">{p.presetName}</div>
                  <div className="flex items-center gap-2"><span className="px-1.5 py-0.5 rounded bg-zinc-800 text-[8px] font-black text-zinc-500 uppercase tracking-widest">{p.game}</span><span className="text-[9px] font-bold text-zinc-600 uppercase tracking-wider">{p.layers.length} Layers</span></div>
                </div>
              ))}
            </div>
          </div>
      ...
      {renderProgress && (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-black/80 backdrop-blur-md">
          <div className="w-[400px] bg-[#111215] border border-orange-500 rounded-2xl p-6 text-center shadow-[0_0_30px_rgba(234,88,12,0.3)]">
            ...
            <div className="w-full h-2 bg-zinc-900 rounded-full overflow-hidden mb-2"><div className="h-full bg-orange-600 transition-all duration-300" style={{ width: `${renderProgress.progress}%` }} /></div>
            <div className="text-orange-500 font-mono text-xs">{Math.round(renderProgress.progress)}%</div>
      ...
      {modal && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/80 backdrop-blur-md">
          <div className="w-[420px] bg-[#111215] border border-white/10 rounded-2xl p-6 shadow-2xl">
            ...
            {modal.type === "prompt" && <input autoFocus className="w-full bg-zinc-900 border border-white/10 rounded-xl px-4 py-3 text-white text-xs mb-6 focus:outline-none focus:border-orange-500 font-bold" defaultValue={modal.defaultValue} onKeyDown={(e) => { if(e.key === 'Enter') modal.onConfirm?.((e.target as HTMLInputElement).value); }} />}
            <div className="flex gap-2">
              ...
              <button onClick={() => { const val = (document.querySelector('input') as HTMLInputElement)?.value; modal.onConfirm?.(val); }} className="flex-1 py-2.5 bg-orange-600 hover:bg-orange-500 text-white font-black text-[10px] rounded-xl uppercase tracking-widest shadow-lg shadow-orange-600/20 transition cursor-pointer">OK</button>
            </div>
          </div>
        </div>
      )}
